...
def adminmanage(request):
    ...
    rolename = models.UserManage.objects.filter(username=username).values("DoManage__username")[0].get(
        "DoManage__username")
...
def adminbanner(request, page):
    ...
    if request.method == 'POST':
        ...
        if upid:
            ...
            if upimg:
                img = SaveImg(upimg)
                models.IndexBanerInfo.objects.filter(id=upid).update(img=img, a=uplink, title=uptitle, p=upcontent,
                                                                     status=upstatus)
...
def adminbullhorn(request, page):
    ...
    if request.method == 'POST':
        ...
        if upid:
            ...
            if upphoto:
                img = SaveImg(upphoto)
                models.IndexBullhornInfo.objects.filter(id=upid).update(img=img, title=uptitle, content=upcontent,
                                                                        publisher=uppublisher, times=uptimes,
                                                                        a_link=uplink, hot_id=uphot_id)
            models.IndexBullhornInfo.objects.filter(id=upid).update(title=uptitle, content=upcontent,
                                                                    publisher=uppublisher, times=uptimes, a_link=uplink,
                                                                    hot_id=uphot_id)
            return HttpResponseRedirect('/admins/bullhorn')
        ...
        sort_id = 1
        models.IndexBullhornInfo(img=img, title=title, content=content, publisher=publisher, times=times, a_link=link,
                                 sort_id=sort_id, hot_id=hot_id).save()
        return HttpResponseRedirect('/admins/bullhorn')
...
def admincase(request, page):
    ...
    if request.method == 'POST':
        ...
        if upid:
            ...
            if upphoto:
                img = SaveImg(upphoto)
                models.IndexCaseInfo.objects.filter(id=upid).update(img=img, title=uptitle, content=upcontent,
                                                                    a_link=uplink)
...
def adminnews(request, page):
    ...
    if request.method == 'POST':
        ...
        if uptitle:
            ...
            models.News.objects.filter(id=upid).update(title=uptitle, content=upcontent, author=upauthor,
                                                       uptime=upuptime)
    ...
    if request.method == 'POST':
        ...
        if upid:
            ...
            if upusername and upemail and upphone:
                models.UserManage.objects.filter(id=upid).update(username=upusername, email=upemail, phone=upphone,
                                                                 DoManage_id=uprole)
            return HttpResponseRedirect('/admins/usermanage')
...
def admindomanage(request, page):
    if request.method == 'GET':
        ...
        ret = {'data': data, "page": PageNum(page, pagecount, "admins/domanage"), "tree": tree,
...
def adminmenumanage(request):
    ...
    if request.method == "POST":
        ...
        if uppermissionsid:
            ...
            models.Permissions.objects.filter(id=uppermissionsid).update(title=upmenuname, url=upmenuurl,
                                                                         MenuTree_id=upmenuid)
    ...
    if request.method == "GET":
        ...
        if docsv:
            ...
            for row in quessydata:
                query = [row.id, (row.name).encode('GB2312'), (row.phone).encode('GB2312'),
                         (row.email).encode('GB2312'), (row.content).encode('GB2312'), str(str(row.createtime)[0:-6])]
                data.append(query)
            return copycsv(data)
...
def adminchartdatas(request):
    ...
    day = {"timelists": timelists, "index": index, "information": information, "case": case, "about": about,
           "news": news, "iplist": iplist, }
